#!/usr/bin/env python3
"""Day 18: Worktree Isolation.

Day 17 的 Agent 已经能在预算内自主推进。本章继续增加 Worktree Isolation：
让 task / subagent 可以在一个隔离的临时工作目录里运行，避免直接改动主工作区。

这个版本故意保持最小：在主工作区下创建 .agent_worktrees/ 目录，每个 worktree 是
一个独立目录（可选 git worktree），并记录路径、状态和关联 task。

    worktree_create -> WorktreeRecord -> task(worktree_id) -> isolated cwd -> worktree_read/list

Worktree Isolation 的目标不是替换 git，而是让 Harness 可以“先在沙盒里试一下”。
"""
...
# TodoWrite 在 Day 18 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 18 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read", "team_list", "team_read", "protocol_list", "autonomous_list", "autonomous_read", "worktree_list", "worktree_read"}
]
...
def main() -> None:
    """启动一个支持 Worktree Isolation 的最小 Agent。"""
    ensure_memory_file()
    ensure_cron_scheduler_started()
    TEAM_PROTOCOLS.update(builtin_team_protocols())
    print("Day 18 Worktree Isolation. 输入任务开始，输入 q / exit / 空行退出。")
