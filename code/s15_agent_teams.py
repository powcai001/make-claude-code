...
Day 14 的 Agent 已经能按时间触发后台任务。本章继续增加 Agent Teams：
把多个带角色的子 Agent 编组成一个团队，让它们分别从不同角度调查同一个问题，
最后由 Harness 汇总成一份团队报告。

这个版本故意保持顺序执行，先把团队、成员、任务分发和结果汇总抽象做清楚：

    team_run -> role prompts -> tracked subagent tasks -> aggregate team report

Agent Teams 让 Harness 不再只会派出一个通用子 Agent，而是可以组织 researcher、reviewer、tester 等角色协作。
"""
...
# TodoWrite 在 Day 15 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 15 policy"
...
def new_team_run(objective: str, members: list[TeamMember]) -> TeamRun:
    """创建一次团队运行记录。"""
    global NEXT_TEAM_ID
    ...
    try:
        for member in team_members:
            member_prompt = (
                ...
                f"Role instructions:\n{member['prompt']}\n\n"
                "Return a concise role-specific report. Do not solve other members' roles unless needed."
            )
            record = new_task_record(
...
SUBAGENT_TOOL_HANDLERS: dict[str, ToolHandler] = {
    ...
    "team_run": lambda **kwargs: run_team(kwargs["objective"], kwargs.get("members")),
}
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "members": {
                    "type": "array",
                    "description": "Optional team members; defaults to researcher/reviewer/tester.",
                    "items": {
                        "type": "object",
                        "properties": {
                            "name": {"type": "string"},
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read", "team_list", "team_read"}
]
...
def main() -> None:
    """启动一个支持 Agent Teams 的最小 Agent。"""
    ensure_memory_file()
    ensure_cron_scheduler_started()
    print("Day 15 Agent Teams. 输入任务开始，输入 q / exit / 空行退出。")
