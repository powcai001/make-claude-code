...
Day 12 的 Agent 已经能记录子任务生命周期。本章继续增加 Background Tasks：
让子 Agent 可以在后台线程里运行，主 Agent 立即拿回 task_id，随后用 task_list、
task_read 或 task_wait 查询进度和结果。

这个版本故意保持本地线程模型，不引入队列服务：

    task(run_in_background=True) -> background thread -> running task -> poll/wait/read

Background Tasks 让 Harness 不必阻塞在长调查上，是并发、取消和进度事件的基础。
"""
...
# TodoWrite 在 Day 13 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 13 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait"}
]
...
def main() -> None:
    """启动一个支持后台任务的最小 Agent。"""
    ensure_memory_file()
    print("Day 13 Background Tasks. 输入任务开始，输入 q / exit / 空行退出。")
