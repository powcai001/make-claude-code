...
Day 11 的 Agent 已经能把错误变成可恢复反馈。本章继续强化 Task System：
把子 Agent 调用从一个普通函数升级成有任务 ID、状态、耗时、轮次和结果记录的
任务生命周期，让主 Agent 可以查看、复盘和引用之前的子任务。

这个版本故意保持同步执行，先把任务管理抽象做清楚：

    task request -> TaskRecord -> running subagent -> completed/failed -> task history

Task System 不是简单地“再开一个模型调用”，而是 Harness 管理并发、隔离、状态和结果的基础。
"""
...
# TodoWrite 在 Day 12 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 12 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read"}
]
...
def main() -> None:
    """启动一个带 Task System 的最小 Agent。"""
    ensure_memory_file()
    print("Day 12 Task System. 输入任务开始，输入 q / exit / 空行退出。")
