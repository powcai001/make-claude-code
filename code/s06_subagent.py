...
Day 05 的 Agent 已经能用 TodoWrite 维护可见计划。本章继续增加
Subagent：主 Agent 可以把一个聚焦的调查任务委派给另一个受控 Agent Loop，
子 Agent 使用独立上下文和受限工具完成研究，最后把结果汇报给主 Agent。

这个版本故意保持简单：

    tool_use(task) -> run_subagent -> subagent report -> tool_result

Subagent 不负责后台并发，也不拥有写文件权限。它只是把“临时调查上下文”
从主对话里隔离出来。
"""
...
# TodoWrite 在 Day 06 仍然只做内存态，后续 Memory 章节再考虑持久化。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 06 policy"
...
def main() -> None:
    ...
    print("Day 06 Subagent. 输入任务开始，输入 q / exit / 空行退出。")
