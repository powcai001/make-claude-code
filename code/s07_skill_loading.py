#!/usr/bin/env python3
"""Day 07: Skill Loading.

Day 06 的 Agent 已经可以通过 Subagent 隔离调查上下文。本章继续增加
Skill Loading：Harness 在每次用户输入后发现可用技能、选择相关技能，并把精简
说明注入 system prompt。

这个版本故意保持简单：

    user prompt -> discover skills -> select skills -> build system prompt

Skill 只是指令包，不是可执行插件。完整技能内容通过 read_skill 按需读取，避免
把所有领域知识一次性塞进上下文。
"""
...
# TodoWrite 在 Day 07 仍然只做内存态，后续 Memory 章节再考虑持久化。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 07 policy"
...
def main() -> None:
    """启动一个带 Skill Loading 的最小 Agent。"""
    print("Day 07 Skill Loading. 输入任务开始，输入 q / exit / 空行退出。")
