...
Day 10 的 Agent 已经能结构化组装 System Prompt。本章继续增加 Error Recovery：
当模型调用、工具执行或参数输入失败时，不让 Agent 直接崩溃，而是分类错误、
记录错误，并把可执行的恢复建议反馈给模型。

这个版本故意保持简单：

    model/tool error -> classify -> record -> recovery hint -> next model turn

Error Recovery 不是假装错误不存在，而是把错误变成模型和 Harness 可以继续处理的结构化反馈。
"""
...
# TodoWrite 在 Day 11 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 11 policy"
...
def main() -> None:
    """启动一个带错误恢复提示的最小 Agent。"""
    ensure_memory_file()
    print("Day 11 Error Recovery. 输入任务开始，输入 q / exit / 空行退出。")
