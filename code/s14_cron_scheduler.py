#!/usr/bin/env python3
"""Day 14: Cron Scheduler.

Day 13 的 Agent 已经能把子任务放到后台线程里运行。本章继续增加 Cron Scheduler：
让 Harness 可以登记周期性任务，在到期时自动启动后台 Task，并记录每次触发。

这个版本故意保持进程内调度，不依赖系统 cron 或外部队列：

    cron_add(interval) -> schedule record -> cron_tick / scheduler loop -> background task

Cron Scheduler 把“用户主动发起任务”扩展成“Harness 按时间触发任务”，是自动巡检、定时总结和长期代理的基础。
"""
...
# TodoWrite 在 Day 14 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 14 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read"}
]
...
def main() -> None:
    """启动一个带 Cron Scheduler 的最小 Agent。"""
    ensure_memory_file()
    ensure_cron_scheduler_started()
    print("Day 14 Cron Scheduler. 输入任务开始，输入 q / exit / 空行退出。")
