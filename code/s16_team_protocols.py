#!/usr/bin/env python3
"""Day 16: Team Protocols.

Day 15 的 Agent 已经能组成角色化小队。本章继续增加 Team Protocols：
把团队协作流程沉淀成可复用的协议模板（plan / debate / redteam / custom），
让 team_run 不再每次靠系统提示临时发挥，而是按协议走固定的多阶段流程。

这个版本保持顺序执行，重点是把协议、阶段和阶段结果串起来：

    team_run(protocol) -> protocol stages -> tracked tasks -> protocol report

Team Protocols 把 Agent Teams 从“多个视角分别调查”升级成“按既定流程协作”。
"""
...
# TodoWrite 在 Day 16 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 16 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read", "team_list", "team_read", "protocol_list"}
]
...
def main() -> None:
    ...
    print("Day 16 Team Protocols. 输入任务开始，输入 q / exit / 空行退出。")
