...
Day 08 的 Agent 已经能在当前会话里自动压缩上下文。本章继续增加 Memory：
把用户偏好、项目约定、重要事实持久化到工作区文件里，让 Agent 跨轮次、
跨进程重启后仍然能记住稳定信息。

这个版本故意保持简单：

    user fact -> remember tool -> memory file -> next system prompt

Memory 和 Context Compact 不同：Compact 是当前会话续航；Memory 是跨会话保留稳定事实。
"""
...
# TodoWrite 在 Day 09 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 09 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory"}
]
...
def main() -> None:
    ...
    ensure_memory_file()
    print("Day 09 Memory. 输入任务开始，输入 q / exit / 空行退出。")
