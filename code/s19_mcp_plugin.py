...
Day 18 的 Agent 已经能把任务放进隔离 worktree 里运行。本章继续增加 MCP Plugin：
让 Harness 可以注册外部能力提供方（plugin），把它们暴露成统一的 mcp_* 工具，
主 Agent 不需要知道能力来自内置函数、工作区插件，还是未来的 MCP server。

这个版本故意不连接真实网络服务，而是实现一个内存态的 MCP plugin registry：

    mcp_register -> McpPlugin -> McpTool -> mcp_call -> plugin result

MCP Plugin 的核心不是协议细节，而是把“外部工具能力”抽象成可发现、可读取、可调用的插件接口。
"""
...
def permission_decision(
    ...
    return "allow", "safe by current Day 19 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read", "team_list", "team_read", "protocol_list", "autonomous_list", "autonomous_read", "worktree_list", "worktree_read", "mcp_list", "mcp_read", "mcp_call"}
]
...
def main() -> None:
    """启动一个支持 MCP Plugin 的最小 Agent。"""
    ensure_memory_file()
    ensure_cron_scheduler_started()
    TEAM_PROTOCOLS.update(builtin_team_protocols())
    ensure_mcp_plugins()
    print("Day 19 MCP Plugin. 输入任务开始，输入 q / exit / 空行退出。")
