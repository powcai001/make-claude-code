#!/usr/bin/env python3
"""Day 08: Context Compact.

Day 07 的 Agent 已经能按任务动态加载 Skill。本章继续增加 Context Compact：
当对话历史越来越长时，Harness 会把较早消息压缩成摘要，同时保留最近窗口，
让当前会话可以继续运行。

这个版本故意保持简单：

    messages -> estimate size -> summarize old messages -> keep recent window

Context Compact 不是长期记忆，也不写入磁盘。它只是当前会话的续航机制。
"""
...
SYSTEM = (
    ...
    "Use task to launch a focused subagent for research or analysis. "
...
# TodoWrite 在 Day 08 仍然只做内存态，后续 Memory 章节再考虑持久化。
TODOS: list[TodoItem] = []
...
def permission_decision(
    ...
    return "allow", "safe by current Day 08 policy"
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill"}
]
...
def main() -> None:
    """启动一个带 Context Compact 的最小 Agent。"""
    print("Day 08 Context Compact. 输入任务开始，输入 q / exit / 空行退出。")
