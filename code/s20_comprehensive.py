#!/usr/bin/env python3
"""Day 20: Comprehensive.

Day 19 的 Agent 已经能通过 MCP Plugin 接入外部能力。本章不再叠加新机制，
而是把前 19 天的能力整合收尾：补一个 system_status 总览工具串起全部子系统，
让主 Agent 和用户都能用一个入口查看 Harness 的全局状态。

    system_status -> tasks / teams / cron / worktrees / autonomy / mcp / memory / skills

Comprehensive 不是“加功能”，而是“让已经造好的机器有一个仪表盘”。
"""
...
Use worktree_create when a subagent should experiment in an isolated directory without touching the main workspace.
Use mcp_list and mcp_call when external plugin capabilities are relevant to the task.
Use system_status to inspect the overall Harness state before deciding next actions.
...
- Remove worktrees when done; do not leave experiments behind in the main workspace.
- Treat MCP plugin tools as external capabilities; validate plugin and tool names before calling."""
...
# TodoWrite 在 Day 19 仍然只做当前会话计划；长期事实交给 Memory。
TODOS: list[TodoItem] = []
...
class CwdStack:
    ...
    默认 cwd 始终是主工作区 WORKDIR；worktree 内运行的任务会临时切换到隔离目录。
    """

    def __init__(self) -> None:
        self._local = threading.local()

    def _stack(self) -> list[Path]:
...
def permission_decision(
    ...
    return "allow", "safe by current Day 20 policy"
...
def run_task(description: str, prompt: str, run_in_background: bool = False, worktree_id: str | None = None) -> str:
    ...
    如果传入了 worktree_id，子 Agent 会在该隔离工作目录里运行，bash 命令的 cwd
    临时切换到 worktree 路径，运行结束后恢复主工作区。
    """
    if not isinstance(description, str) or not description.strip():
    ...
    if worktree_id:
        with WORKTREE_LOCK:
            wt = WORKTREES.get(worktree_id)
        if wt is None:
            return f"Error: worktree {worktree_id!r} not found."
        if wt["status"] != "open":
            return f"Error: worktree {worktree_id} is {wt['status']}."
        worktree_path = wt["path"]

    record = new_task_record(description, prompt, background=bool(run_in_background))
    if worktree_path:
        record["description"] = f"{record['description']} [worktree:{worktree_id}]"
        with WORKTREE_LOCK:
            wt and wt["task_ids"].append(record["id"])
        prompt = (
            f"You are running inside an isolated worktree at {worktree_path}. "
            "Treat that directory as your working directory. "
            "Do not assume files from the main workspace are present unless you create them.\n\n" + prompt
        )
        record["prompt"] = prompt
    if run_in_background:
        thread = threading.Thread(
            target=run_background_task,
            args=(record, worktree_path),
...
def is_inside_workspace(path: Path) -> bool:
    """判断路径是否安全地落在主工作区内，避免 worktree 逃逸到工作区之外。"""
    try:
...
def detect_base_ref() -> str | None:
    """尝试探测当前 git 分支或 HEAD；失败时返回 None。"""
    try:
        result = subprocess.run(
...
def new_worktree_record(use_git: bool = False, base_ref: str | None = None) -> WorktreeRecord:
    """创建隔离工作目录并返回记录。"""
    global NEXT_WORKTREE_ID
    wt_id = f"wt-{NEXT_WORKTREE_ID:04d}"
    NEXT_WORKTREE_ID += 1
    ...
    with WORKTREE_LOCK:
        WORKTREES[wt_id] = record
    return record


def format_worktree_record(record: WorktreeRecord) -> str:
    """把 worktree 记录渲染成可读文本。"""
    lines = [
        f"{record['id']} [{record['status']}]",
        f"path: {record['path']}",
        f"created: {record['created_at']}",
        f"is_git_worktree: {record['is_git_worktree']}",
        f"base_ref: {record['base_ref'] or '-'}",
        f"task_ids: {', '.join(record['task_ids']) or '-'}",
    ]
    return "\n".join(lines)


def run_worktree_create(use_git: bool = False, base_ref: str | None = None) -> str:
    """工具：创建一个隔离工作目录。"""
    record = new_worktree_record(use_git=use_git, base_ref=base_ref)
    return f"Created worktree {record['id']}.\n\n{format_worktree_record(record)}"
...
def run_worktree_list() -> str:
    ...
    with WORKTREE_LOCK:
        if not WORKTREES:
            return "No worktrees have been created yet."
        records = list(WORKTREES.values())
    return "\n\n".join(format_worktree_record(record) for record in records)


def run_worktree_read(wt_id: str) -> str:
    """工具：读取一个 worktree 详情。"""
    with WORKTREE_LOCK:
        record = WORKTREES.get(wt_id)
    if record is None:
        return f"Error: worktree {wt_id!r} not found."
    return format_worktree_record(record)


def run_worktree_remove(wt_id: str) -> str:
    """工具：移除一个 worktree，清理它的目录。"""
    with WORKTREE_LOCK:
        record = WORKTREES.get(wt_id)
        if record is None:
            return f"Error: worktree {wt_id!r} not found."
        if record["status"] == "removed":
            return f"Worktree {wt_id} already removed."
...
def ensure_mcp_plugins() -> None:
    """确保内置 MCP 插件已注册。"""
    with MCP_LOCK:
        if not MCP_PLUGINS:
            MCP_PLUGINS.update(builtin_mcp_plugins())


def run_mcp_register(name: str, description: str = "") -> str:
    """工具：注册一个内存态 MCP 插件占位符。"""
    if not isinstance(name, str) or not name.strip():
        return "Error: plugin name must be a non-empty string."
    key = name.strip()
    with MCP_LOCK:
        if key in MCP_PLUGINS:
            return f"Error: MCP plugin {key!r} already exists."
        MCP_PLUGINS[key] = {
            "name": key,
            "description": description.strip() or "User-registered MCP plugin placeholder.",
            "enabled": True,
            "tools": {},
        }
    return f"Registered MCP plugin {key}."
...
def run_mcp_read(plugin: str) -> str:
    ...
    lines = [f"{record['name']} [{'enabled' if record['enabled'] else 'disabled'}]", record["description"], "tools:"]
...
def run_mcp_call(plugin: str, tool: str, arguments: dict[str, Any] | None = None) -> str:
    ...
    if plugin == "workspace" and tool == "stats":
        return "\n".join(
            [
                f"tasks: {len(TASKS)}",
                f"teams: {len(TEAM_RUNS)}",
                f"cron_jobs: {len(CRON_JOBS)}",
                f"worktrees: {len(WORKTREES)}",
                f"autonomous_runs: {len(AUTONOMY_RUNS)}",
                f"args: {args}",
            ]
        )
    return f"MCP plugin {plugin}.{tool} is registered but has no local handler in this minimal demo."


def run_mcp_enable(plugin: str, enabled: bool = True) -> str:
    """工具：启用或禁用 MCP 插件。"""
    ensure_mcp_plugins()
    with MCP_LOCK:
        record = MCP_PLUGINS.get(plugin)
    ...
    return f"MCP plugin {plugin} is now {'enabled' if enabled else 'disabled'}."
...
def run_system_status() -> str:
    ...
    memory_lines = read_project_memory().strip().splitlines()
    memory_entries = max(0, len([line for line in memory_lines if line.startswith("- ")]))

    sections: list[str] = []
    sections.append("# Harness System Status")
    sections.append("")
    sections.append("## Counts")
    sections.append(f"- todos: {len(TODOS)}")
    sections.append(f"- skills loaded: {len(LOADED_SKILLS)}")
    sections.append(f"- memory entries: {memory_entries}")
    sections.append(f"- recent errors: {len(ERROR_HISTORY)}/{MAX_ERROR_HISTORY}")
    sections.append(f"- tasks: {tasks_total} {tasks_by_status} (background threads alive: {threads_alive})")
    sections.append(f"- cron jobs: {cron_total} {cron_by_status}")
    sections.append(f"- team runs: {teams_total} {teams_by_status}")
    sections.append(f"- autonomous runs: {autonomy_total} {autonomy_by_status}")
    sections.append(f"- worktrees: {worktree_total} {worktree_by_status}")
    sections.append(f"- mcp plugins: {mcp_total} (enabled: {mcp_enabled})")
    sections.append(f"- protocols: {len(TEAM_PROTOCOLS)}")

    scheduler_alive = CRON_SCHEDULER_THREAD is not None and CRON_SCHEDULER_THREAD.is_alive()
    sections.append("")
    sections.append("## Subsystems")
    sections.append(f"- cron scheduler thread: {'running' if scheduler_alive else 'stopped'}")
    sections.append(f"- effective cwd: {_CWD_STACK.current()}")
    sections.append(f"- model: {MODEL or '(unset)'}")
    sections.append(f"- workspace: {WORKDIR}")
    return "\n".join(sections)
...
SUBAGENT_TOOL_HANDLERS: dict[str, ToolHandler] = {
    ...
    "worktree_create": lambda **kwargs: run_worktree_create(kwargs.get("use_git", False), kwargs.get("base_ref")),
    "worktree_list": lambda **kwargs: run_worktree_list(),
    "worktree_read": lambda **kwargs: run_worktree_read(kwargs["wt_id"]),
    "worktree_remove": lambda **kwargs: run_worktree_remove(kwargs["wt_id"]),
    "mcp_list": lambda **kwargs: run_mcp_list(),
    "mcp_read": lambda **kwargs: run_mcp_read(kwargs["plugin"]),
    "mcp_call": lambda **kwargs: run_mcp_call(kwargs["plugin"], kwargs["tool"], kwargs.get("arguments")),
    "mcp_enable": lambda **kwargs: run_mcp_enable(kwargs["plugin"], kwargs.get("enabled", True)),
    "mcp_register": lambda **kwargs: run_mcp_register(kwargs["name"], kwargs.get("description", "")),
...
TOOL_HANDLERS: dict[str, ToolHandler] = {
    ...
    "task": lambda **kwargs: run_task(kwargs["description"], kwargs["prompt"], kwargs.get("run_in_background", False), kwargs.get("worktree_id")),
    "list_skills": lambda **kwargs: run_list_skills(),
    "read_skill": lambda **kwargs: run_read_skill(kwargs["name"]),
    "read_memory": lambda **kwargs: run_read_memory(),
...
TOOLS = [
    ...
    },
    {
        ...
        "description": "Create an isolated worktree directory under .agent_worktrees for experiments.",
        "input_schema": {
            "type": "object",
            "properties": {
                "use_git": {"type": "boolean", "description": "If true, create a git worktree from base_ref."},
                "base_ref": {"type": "string", "description": "Optional git ref for the worktree; defaults to current HEAD."},
            },
        },
    },
    {
        "name": "worktree_list",
        "description": "List isolated worktrees and their status.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "worktree_read",
        "description": "Read one worktree by wt_id.",
        "input_schema": {
            "type": "object",
            "properties": {"wt_id": {"type": "string", "description": "Worktree id, for example wt-0001."}},
            "required": ["wt_id"],
        },
    },
    {
        "name": "worktree_remove",
        "description": "Remove an isolated worktree and clean up its directory.",
        "input_schema": {
            "type": "object",
            "properties": {"wt_id": {"type": "string", "description": "Worktree id to remove."}},
            "required": ["wt_id"],
        },
    },
    {
        "name": "mcp_list",
        "description": "List registered MCP plugins and their tools.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "mcp_read",
        "description": "Read one MCP plugin by name.",
        "input_schema": {
            "type": "object",
            "properties": {"plugin": {"type": "string", "description": "MCP plugin name."}},
            "required": ["plugin"],
        },
    },
    {
        "name": "mcp_call",
        "description": "Call one MCP plugin tool by plugin and tool name.",
        "input_schema": {
            "type": "object",
            "properties": {
                "plugin": {"type": "string", "description": "MCP plugin name."},
                "tool": {"type": "string", "description": "Tool name inside the plugin."},
                "arguments": {"type": "object", "description": "Optional tool arguments."},
            },
    ...
    {
        "name": "mcp_enable",
        "description": "Enable or disable an MCP plugin.",
        "input_schema": {
            "type": "object",
            "properties": {
                "plugin": {"type": "string", "description": "MCP plugin name."},
    ...
    {
        "name": "mcp_register",
        "description": "Register an in-memory placeholder MCP plugin.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "system_status",
        "description": "Show a comprehensive overview of the Harness state across all subsystems.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
...
SUBAGENT_TOOLS = [
    tool for tool in TOOLS if tool["name"] in {"bash", "read_file", "list_skills", "read_skill", "read_memory", "show_system_prompt", "show_errors", "task_list", "task_read", "task_wait", "cron_list", "cron_read", "team_list", "team_read", "protocol_list", "autonomous_list", "autonomous_read", "worktree_list", "worktree_read", "mcp_list", "mcp_read", "mcp_call", "system_status"}
]
...
def main() -> None:
    """启动一个完整集成的最小 Agent（Day 20 终章）。"""
    ensure_memory_file()
    ensure_cron_scheduler_started()
    TEAM_PROTOCOLS.update(builtin_team_protocols())
    ensure_mcp_plugins()
    print("Day 20 Comprehensive. 输入任务开始，输入 q / exit / 空行退出。")
