...
Day 02 的 Agent 已经有了多个工具。本章不改工具分发层，只在工具执行前加一道门：

    tool_use -> permission check -> tool handler -> tool_result

Permission 是 Harness 的边界：模型可以提出动作，但是否执行由 Harness 决定。
"""
...
SYSTEM = (
    ...
    "Before tools run, the harness may ask for permission or deny unsafe actions."
)
...
def run_bash(command: str) -> str:
    """执行 shell 命令。权限判断不写在这里，而是在执行前统一处理。"""
    try:
...
def agent_loop(messages: list[dict[str, Any]]) -> None:
    """Agent Loop 仍然只负责分发；权限检查包在 handler 前面。"""
    if not MODEL:
...
def main() -> None:
    """启动一个带权限边界的最小 Agent。"""
    print("Day 03 Permission. 输入任务开始，输入 q / exit / 空行退出。")
