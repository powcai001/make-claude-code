...
所以 Day 12 我把 Subagent 调用升级成一个最小 Task System。
...
Day 12 继续基于 Day 11，所以 Error Recovery、System Prompt、Memory、Skill、Compact 等机制都保留。新增内容集中在任务记录和任务查询。
...
Day 11 里 `run_subagent()` 接收的是 `description` 和 `prompt`。

Day 12 改成接收一个 `TaskRecord`：
...
注意这里和 Day 11 的 Error Recovery 接上了：如果子任务抛异常，会被标记为 `failed`，并通过 `attach_recovery_hint()` 返回恢复建议。
...
如果把它们混在一起，Todo 列表会被子任务生命周期污染，Task 结果也会缺少执行细节。所以 Day 12 我单独引入 `TASKS`，不复用 `TODOS`。

### 坑 2：只返回报告不够

Day 06 的 `task` 工具只返回子 Agent 报告。

但真实使用时，主 Agent 经常需要知道：这个报告来自哪个任务？跑了几轮？有没有失败？能不能稍后再看？

所以 Day 12 的 `run_task()` 返回里明确包含任务 ID，并提示可以用 `task_read` 查看完整记录。
...
真实 Claude Code / Codex 类工具里的 Task System 通常对应这些能力：
