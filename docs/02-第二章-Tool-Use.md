...
所以第 02 章要解决的问题是：把外部能力从一个粗糙的 `bash`，拆成更明确的结构化工具——让模型只表达"做什么"，把"怎么做"交回给 Harness。
...
第一章我实现了 Agent Loop；第 02 章在这个循环上加了工具分发层：把粗糙的 `bash`，拆成 `read_file` / `write_file` / `edit_file` 这些结构化工具。
